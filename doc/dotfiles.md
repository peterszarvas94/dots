...
Theme runtime setup is handled by package deploy flows (not a separate `--theme` flag).

## `stow/` packages

Current packages:

- `ghostty`, `git`, `herdr`, `hypr`, `jj`, `lazygit`, `nvim`, `nvim-theme-mac`, `omarchy`, `opencode`, `pi`, `scripts`
- `ssh`, `systemd`, `tmux`, `xdg`, `zed`, `zsh`

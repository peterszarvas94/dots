...
local state = _G.__theme_runtime_state or { instances = {} }
_G.__theme_runtime_state = state
...
local function normalize_theme(data, fallback)
  ...
  for _, spec in ipairs(data) do
    if spec[1] == 'LazyVim/LazyVim' and spec.opts and type(spec.opts.colorscheme) == 'string' then
      local background = spec.opts.background
      if background ~= 'light' and background ~= 'dark' then
        background = spec.opts.colorscheme == 'gruvbox' and 'dark' or fallback.background
      end
...
function M.sync(name, force)
  local instance = state.instances[name]
...
function M.setup(opts)
  ...
  local instance = state.instances[name] or {}
  instance.name = name
  ...
  vim.api.nvim_create_user_command(command_name, function(cmd_opts)
    local theme, changed = M.sync(name, cmd_opts.bang)
    ...
    bang = true,
    desc = opts.command_desc or 'Sync Neovim theme from generated theme file (! forces reload)',
  })

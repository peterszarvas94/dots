...
function initialization() {
	source "$DIR/config.conf" || source "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/config.conf"
...
function rofi_cmd() {
	...
	# rofi -dmenu -p "Wifi" -width 40 -i -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" $3 -theme "$RASI_DIR" -theme-str 'window{width: '"$WIDTH"'em;}textbox-prompt-colon{str:"'"$PROMPT"':";}'"$4"''
	rofi -dmenu -p "Wifi" -width 40
}
